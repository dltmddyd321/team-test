...
class MainActivity : AppCompatActivity() {
    ...
    private fun openDialog() {
        ...
        openDialog.setIcon(R.drawable.ic_launcher_background)
        openDialog.setTitle("Choose your Image in")
        openDialog.setPositiveButton("Camera") { dialog, _ ->
            openCamera()
            dialog.dismiss()
        }
        openDialog.setNeutralButton("Gallery") { dialog, _ ->
            openGallery()
            dialog.dismiss()
        }
        openDialog.setNegativeButton("Cancel") { dialog, _ ->
            dialog.dismiss()
        }
        openDialog.create()
        openDialog.show()
    ...
    private fun openCamera() {
        ...
        file = File(Environment.getExternalStorageDirectory(),"file" + System.currentTimeMillis().toString() + ".jpg")
    ...
    private fun enableRuntimePermission() {
        if(ActivityCompat.shouldShowRequestPermissionRationale(
            ...
            Toast.makeText(this, "Camera Permission allows us to Camera App",Toast.LENGTH_SHORT).show()
        }else{
    ...
    override fun onRequestPermissionsResult(
        ...
        when(requestCode){
            RequestPermissionCode -> if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                Toast.makeText(this, "Permission Granted, Now your application can access Camera",Toast.LENGTH_SHORT)
                    .show()
            }
            else{
                Toast.makeText(this, "Permission Granted, Now your application can access Camera",Toast.LENGTH_SHORT)
